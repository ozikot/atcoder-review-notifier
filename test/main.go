...
func main() {
	...
	args := flag.Args()
	if len(args) == 0 {
		log.Println("specify command line arguments")
		return
	}

	// get user_id: args[0] submission data
	filtered, err := GetSubmissionData(args[0])
	if err != nil {
		log.Fatal(err)
		return
	}
	...
	ts, err := PostMessage("今週の復習問題を通知します", "", args[1], args[2], "")
	if err != nil {
		log.Fatal(err)
		return
	}
	...
	for _, data := range filtered {
		...
		ts, err = PostMessage(mapProblem[data.ProblemID], msgUrl, args[1], args[2], ts)
	}
}
